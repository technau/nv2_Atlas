###############################
# This script produces the elements to generate all figures. Figures were then imported into Adobe Illustrator for final published composition.
###############################
...
load(file='AlldataDEGS.updated.RData')
...
for (c in 1:length(data1.subsets))
{
  ...
  p1=DimPlot(Alldata.Nv2,cells.highlight=colnames(data1),cols=c('grey80',clust.cp.separate[c]),raster=F)+NoAxes()+NoLegend()+labs(title='A',subtitle=names(data1.subsets[c]))
  # +
  p2= DimPlot(data1,group.by='orig.ident',cols=LibCP,raster=F)&NoAxes()&labs(tag = 'B')&theme(legend.position = 'left')
  
  # Plot subset with clustering
  p3=DimPlot(data1,cols=clust.cp,raster=F)+NoAxes()+NoLegend()+labs(tag = 'D')
  ...
  #generate a collated list of unique DE genes
  {
    ...
    DEG.plot=DotPlot(data1, 'RNA', features = unique(c(alldata.markers$gene[alldata.markers$cluster==names(data1.subsets)[c]][1:10],list)), 
      ...
      RotatedAxis() +FontSize(8,8) +theme(panel.grid = element_line(size=0.002,colour = 'grey'),legend.position = 'bottom',legend.title = element_text(size = 6),legend.text = element_text(size=6),legend.box = 'horizontal')+
    ...
    TF.plot=
      DotPlot(data1,'RNA', features = unique(c(list_TF)), 
              ...
              col.min = 0, col.max = 2.5,
              cols = c('slateblue3','darkorange3'),split.by = 'lifehistory') + 
      RotatedAxis() +FontSize(8,8) +theme(panel.grid = element_line(size=0.002,colour = 'grey'),legend.position = 'bottom',
  ...
  S.layout=c(area(1,1,1,1),area(1,2,1,3),area(2,1,2,2),area(2,3,2,3),area(3,1,4,3),area(5,1,6,3))
  fig.allscale[[c]]=
    p1+p2+dist.clust2+p3+DEG.plot+TF.plot+plot_layout(design = S.layout)
}  
cl=c(9,8,7,12,11,1,2,4,5)

pdf(file='C:/Users/agcol/Documents/manuscripts/Nv2Atlas/FinZ_Revision/supplement.updated.pdf',width=12,height=24,onefile=T)
for (c in 1:9)
  print(fig.allscale[[cl[c]]]+labs(caption = paste('Figure S',c,sep = '')))
dev.off()  
...
#generate lists for all subsets
markers = NULL
markers.TF = NULL#
for (j in 1:length(names(data1.subsets)))
{
...
# generate colour scale for DotPlots: not included in the Seurat Funciton
blue=DotPlot(Alldata.Nv2,'Tpm2',col.min = 0,
             col.max = 2.5,
             cols = c('grey','slateblue3','darkorange3'))
  leg <- ggpubr::get_legend(blue+theme(legend.position = 'right',legend.text.align = 1))
  leg.blue=ggpubr::as_ggplot(leg)
# Convert to a ggplot and print
  orange=DotPlot(Alldata.Nv2,'Tpm2',col.min = 0,
                 col.max = 2.5,
                 cols = c('grey','darkorange3'))
  leg <- ggpubr::get_legend(orange+theme(legend.position = 'right',legend.text.align = 1))
  leg.orange=ggpubr::as_ggplot(leg)
leg.blue+leg.orange

##**Fig1a** cell plot
#First, merge the neurons and neurogland, and the cnidocytes:
levels(Alldata.Nv2$IDs)=c("pSC", "PGCs", "neurogland.all", "gland.mucous", "unchar.immune","neurogland.all", "cnidocyte","cnidocyte", "pharyngeal.ect", "epithelia.ect","ectoderm.embryonic", "retractor muscle", "gastrodermis", "endomesoderm.embryonic")

Alldata.Nv2$IDs=droplevels(Alldata.Nv2$IDs)
Alldata.Nv2$IDs=factor(Alldata.Nv2$IDs,levels(Alldata.Nv2$IDs)[c(1:6,10,12,11,7,9,8)])
Alldata.Nv2=SetIdent(Alldata.Nv2,value='IDs')

clust.cp.IDs=clust.cp.separate[1:12]
names(clust.cp.IDs)=levels(Alldata.Nv2)
cellID.dev = NULL
data2=subset(SetIdent(Alldata.Nv2,value = 'lifehistory'),idents='DevSubset')
data2=SetIdent(data2,value='IDs')
for (i in 1:length(levels(data2)))
  cellID.dev[[i]] = WhichCells(data2, idents = levels(data2)[i])
names(cellID.dev)=levels(data2)
cellID.adult = NULL
data2=subset(SetIdent(Alldata.Nv2,value = 'lifehistory'),idents='AdultSubset')
data2=SetIdent(data2,value='IDs')
for (i in 1:length(levels(data2)))
  cellID.adult[[i]] = WhichCells(data2, idents = levels(data2)[i])
names(cellID.adult)=levels(data2)
ci=sort(names(clust.cp.IDs),index.return=T)
Fig1a=DimPlot(Alldata.Nv2,label=T,cells.highlight = cellID.dev,cols.highlight  = clust.cp.IDs[rev(ci$ix)],sizes.highlight = 0.5)&DimPlot(Alldata.Nv2,label=T,cells.highlight = cellID.adult,cols.highlight  = clust.cp.IDs[rev(ci$ix)],sizes.highlight = 0.5)&NoAxes()&NoLegend()
Fig1a
...
#**fig 1cd**
{
  ...
  {
    x = alldata.markers[as.numeric(alldata.markers$cluster) == i, ][1:min(2, length(which(as.numeric(alldata.markers$cluster) == i))), 7]
  ...
  DEG.plot = DotPlot(
    ...
    RotatedAxis() + FontSize(12, 4) +theme(panel.grid = element_line(size=0.002,colour = 'grey'),legend.position = 'bottom')+ #coord_flip()+
    # NoLegend()+
    labs(title = 'Top 2 DEGs', subtitle = 'p-val < 0.001')
  
  list_TF = NULL
  for (i in 1:length(levels(Alldata.Nv2@active.ident)))
  {
    x = alldata.markers.TF[as.numeric(alldata.markers.TF$cluster) == i, ][1:min(2, length(which(as.numeric(alldata.markers.TF$cluster) == i))), 7]
    list_TF = c(list_TF, x)
  }
  list_TF = unique(list_TF)
  TF.plot = DotPlot(
    ...
    RotatedAxis() + FontSize(12, 4) + NoLegend() + #coord_flip()+
    labs(title = 'Top 2 TFs', subtitle = 'p-val < 0.001')+theme(panel.grid = element_line(size=0.002,colour = 'grey'),legend.position = 'bottom')
  
  
  DEG.plot+TF.plot+plot_layout(ncol=2)
}
...
all.markers_TF.ectoderm=markers.TF$all.ectoderm
...
pal.bands(clust.cp)
Fig2B.1=DimPlot(Alldata.Nv2,cells.highlight = ectoderm@assays$RNA@counts@Dimnames[[2]],
                cols.highlight = clust.cp[7])+NoLegend()+NoAxes()#.split$developmental.series
Fig2B=DimPlot(ectoderm,group.by = 'orig.ident',cols = LibCP)+NoAxes()+NoLegend()
Fig2Balt=DimPlot(ectoderm,cols = ect.clust.cp,label=T)+NoAxes()+NoLegend()
...
leg
FigA1=FeaturePlot(ectoderm,'FoxA',order=T,cols=gene.cp)+NoAxes()+NoLegend()
FigA2=FeaturePlot(ectoderm,'Wnt2',order=T,cols=gene.cp)+NoAxes()+NoLegend()
FigA3=FeaturePlot(ectoderm,'Six3-6',order=T,cols=gene.cp)+NoAxes()+NoLegend()
...
{     #generate a collated list of unique DE genes
  ...
  {
    x=all.markers.ectoderm[as.numeric(all.markers.ectoderm$cluster)==i,][1:min(2,length(which(as.numeric(all.markers.ectoderm$cluster)==i))),7]
      list=c(list,x)
  }
  
  #Image the list
  DEG.plot=DotPlot(ectoderm, 'RNA',features = unique(c(list)), 
                   scale.by='size' , col.min = 0, col.max = 2.5, dot.min = 0.1, 
                   cols = c('slateblue3','darkorange3'),split.by = 'lifehistory'
  ) + 
    RotatedAxis() +FontSize(10,0) +#coord_flip()+
    NoLegend()+
    labs(title = 'Top 2 DEGs',subtitle = 'p-val < 0.001')+theme(panel.grid = element_line(size=0.002,colour = 'grey'),legend.position = 'bottom')
  
  list_TF = NULL
  for (i in 1:length(levels(ectoderm@active.ident)))
  {
    x=all.markers_TF.ectoderm[as.numeric(all.markers_TF.ectoderm$cluster)==i,][1:min(2,length(which(as.numeric(all.markers_TF.ectoderm$cluster)==i))),7]
      list_TF=c(list_TF,x)
  }
  list_TF=unique(list_TF)
  TF.plot=DotPlot(ectoderm, 'RNA',features = unique(c(list_TF)), 
                  ...
                  cols = c('slateblue3','darkorange3'),split.by = 'lifehistory') + 
    RotatedAxis() +FontSize(10,0) +NoLegend()+#coord_flip()+
    labs(title = 'Top 2 TFs',subtitle = 'p-val < 0.001')+theme(panel.grid = element_line(size=0.002,colour = 'grey'),legend.position = 'bottom')
...
FeaturePlot(ao,'ISX-like-1',order=T)+scale_color_gradientn(colours = gene.cp)&NoAxes()&NoLegend()
FeaturePlot(ao,'POXA-like-1',order=T)+scale_color_gradientn(colours = gene.cp)&NoAxes()&NoLegend()
DimPlot(ao,cols=ao.cp,pt.size = 2)&NoAxes()&NoLegend()
DimPlot(ao,group.by = 'orig.ident',cols=LibCP,pt.size = 2)&NoAxes()&NoLegend()
...
endoderm=data1.subsets$all.gastrodermis
  ...
  Fig4Ai=DimPlot(Alldata.Nv2,cells.highlight = endoderm@assays$RNA@counts@Dimnames[[2]],
                 cols.highlight = LibCP[5] )+NoLegend()+NoAxes()#.split$developmental.series
  Fig4A=DimPlot(endoderm,group.by = 'orig.ident',cols = LibCP)+NoAxes()+NoLegend()
  ...
  Fig4i = DimPlot(endoderm,cols = clust.cp)+NoAxes()+NoLegend()
  ...
  {     #generate a collated list of unique DE genes
    ...
    {
      x=all.markers.endoderm[as.numeric(all.markers.endoderm$cluster)==i,][1:min(2,length(which(as.numeric(all.markers.endoderm$cluster)==i))),7]
    ...
    DEG.plot=DotPlot(endoderm, features = unique(c(list)), 
      ...
      RotatedAxis() +FontSize(10,6) +#coord_flip()+
      NoLegend()+
      labs(title = 'Top 2 DEGs',subtitle = 'p-val < 0.001')+theme(panel.grid = element_line(size=0.002,colour = 'grey'),legend.position = 'bottom')
    ...
    for (i in 1:length(levels(endoderm@active.ident)))
    {
      x=all.markers_TF.endoderm[as.numeric(all.markers_TF.endoderm$cluster)==i,][1:min(2,length(which(as.numeric(all.markers_TF.endoderm$cluster)==i))),7]
        list_TF=c(list_TF,x)
    }
    list_TF=unique(list_TF)
    TF.plot=DotPlot(endoderm, features = unique(c(list_TF)), 
                    ...
                    col.min = 0, col.max = 3,  dot.min = 0.1,
                    cols = c('slateblue3','darkorange3'),split.by = 'lifehistory') + 
      RotatedAxis() +FontSize(10,6) +NoLegend()+#coord_flip()+
      labs(title = 'Top 2 TFs',subtitle = 'p-val < 0.001')+theme(panel.grid = element_line(size=0.002,colour = 'grey'),legend.position = 'bottom')
...
#**Figure 4**
fig4.layout=c(area(1,1),area(1,2),area(2,1),area(2,2),area(4,1,4,2),area(3,1,3,2))
pdf(file='D:/Alison/manuscripts/Nv2Atlas/RDataFigures/Fig4.raw.pdf',width=16,height = 24,onefile=T)
print(Fig4Ai+Fig4i+Fig4A+Fig4B+TF.plot+DEG.plot+NoLegend()+DEG.plot+TF.plot+plot_layout(design = fig4.layout))
dev.off()
...

{
  ...
  #generate a collated list of unique DE genes
  {
    ...
    for (i in 1:length(levels(data1@active.ident)))
    {
      x=all.markers[as.numeric(all.markers$cluster)==i,][1:min(2,length(which(as.numeric(all.markers$cluster)==i))),7]
        list=c(list,x)
    }
    
    #Image the list
    DEG.plot=DotPlot(data1, 'RNA', features = unique(c(alldata.markers$gene[alldata.markers$cluster=='cnidocyte'][1:5],list)), 
                     scale.by='radius' , dot.min = 0.1,
                     col.min = 0, col.max = 2.5,
                     cols = c('slateblue3','darkorange3'),split.by = 'lifehistory') + 
      RotatedAxis() +FontSize(10,0) +theme(panel.grid = element_line(size=0.002,colour = 'grey'),legend.position = 'bottom',
...
pdf(file='D:/Alison/manuscripts/Nv2Atlas/RDataFigures/Fig6.raw.pdf',width=16,height = 24,onefile=T)
...
dist.clust2=
  ...
  FeaturePlot(data1,c('NUSAP-like-1'),order=T,raster=F)+scale_color_gradientn(colours=gene.cp)&NoAxes()&NoLegend()
FeaturePlot(data1,c('PCNA'),order=T,raster=F)+scale_color_gradientn(colours=gene.cp)&NoAxes()&NoLegend()
FeaturePlot(data1,c('SoxC'),order=T,raster=F)+scale_color_gradientn(colours=gene.cp)&NoAxes()&NoLegend()
FeaturePlot(data1,c('Nanos1'),order=T,raster=F)+scale_color_gradientn(colours=gene.cp)&NoAxes()&NoLegend()
...
{list = NULL
  ...
  DEG.plot=DotPlot(data1, 'RNA', features = unique(list), 
                   ...
                   col.min = 0, col.max = 2.5,
                   cols = c('slateblue3','darkorange3'),split.by = 'lifehistory') + 
    RotatedAxis() +FontSize(10,1) +theme(panel.grid = element_line(size=0.002,colour = 'grey'),legend.position = 'bottom',
  ...
  for (i in 1:length(levels(data1@active.ident)))
  {
    x=all.markers_TF[as.numeric(all.markers_TF$cluster)==i,][1:min(2,length(which(as.numeric(all.markers_TF$cluster)==i))),7]
      list_TF=c(list_TF,x)
  }
  list_TF=unique(list_TF)
  TF.plot=
...
pdf(file='D:/Alison/manuscripts/Nv2Atlas/RDataFigures/Fig7.raw.revise.pdf',width=16,height = 26,onefile=T)
...
{  if (!exists("markers"))
  ...
  for (i in 1:length(levels(data1@active.ident)))
  {
    x = all.markers[as.numeric(all.markers$cluster) == i, ][1:min(1, length(which(as.numeric(all.markers$cluster) == i))), 7]
  ...
  for (i in 1:length(levels(data1@active.ident)))
  {
    x = all.markers_TF[as.numeric(all.markers_TF$cluster) == i, ][1:min(1, length(which(as.numeric(all.markers_TF$cluster) == i))), 7]
    list_TF = c(list_TF, x)
  }
  list_TF = unique(list_TF)
  ...
  DEG.plot = DotPlot(
    data1,'RNA',
    features = unique(c('SoxC','SoxB.2',list_TF,list)),
    scale.by = 'size' ,
    dot.min = 0.1,
    col.min = 0,
    col.max = 2.5,
    split.by = 'lifehistory',
    cols = c('slateblue3', 'darkorange'),dot.scale = 2.5
    # cols = c('lightgrey', 'red')
  ) +
    RotatedAxis() + FontSize(8, 8) + #coord_flip()+
    # NoLegend()+
    labs( title = 'Top differentially expressed genes', subtitle = 'p-val < 0.001')+theme(panel.grid = element_line(size=0.002,colour = 'grey'),legend.position = 'bottom')+coord_flip()
  
  }
DEG.plot

#generate a plot area setup:
Fig8_9.layout=c(area(1,1),area(1,2,1,3),area(1,4,1,5),area(2,1,3,5),area(4,1,5,5))

pdf(file='D:/Alison/manuscripts/Nv2Atlas/RDataFigures/Fig8.raw.pdf',width=18,height = 26,onefile=T)
